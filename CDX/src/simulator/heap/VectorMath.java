...
/**
 * The <code>VectorMath</code> class implements the mathematical functions for
 * manipulating vectors of two and three dimensions. Its operators are patterned
 * after three address code machines, specifying two operands and a destination
 * operand. This machine is optimized for performance, thus the methods are
 * static, contain no calls, create no objects, and do not perform error
 * checking or synchronization.
 * 
 * @author Ben L. Titzer
 */
public final class VectorMath {
	...
	/**
	 * The <code>add</code> method takes two vectors and adds them, placing
	...
	/**
	 * The <code>scale</code> method takes a <code>Vector3d</code> and a
	 * scalar float value multiplies each component of the Vector, storing the
	...
	/**
	 * The <code>normalize</code> method takes a <code>Vector3d</code> and
	 * if it is non-zero, will normalize it so that its magnitude will be 1.
	 * 
	 * @param a
	 ...
	 * the <code>distance</code> method that avoids an expensive floating
	 * point square root computation.
	 * 
	 * @param a
	 *            the value of the first vector
	 * @param b
	...
	/**
	 * The <code>rotate</code> method takes a <code>Vector3d</code> and a
	 * scalar float value and will rotate the vector in the xy plane.
	 * 
	 * @param a
	 *            the value of the first vector
	 * @param radians
	...
	/**
	 * The <code>theta</code> method takes a <code>Vector3d</code> and
	 * calculates the angle between the X-axis and the vector, ignoring the z
	 * component of the vector.
	 * 
	 * @param a
	 *            the vector of which to calculate the theta angle
	 * @returns the radian value in the range [0, 2*pi] that represents the
	 *          angle between the x axis and this vector (in the xy plane)
	 * @throws ZeroVectorException
	 *             if the vector passed equals the zero vector, for which the
	 *             theta value is undefined
	 */
	public static float theta(Vector3d a) {
		...
		if (x < 0)
			return (float) Math.PI - t; // adjust quadrant
		if (t < 0)
			t += 2 * Math.PI; // range adjustment [0, 2*pi]

		return t;
	}

	/**
	 * The <code>phi</code> method takes a <code>Vector3d</code> and
	 * calculates the elevation between the XY-plane and the vector.
	 * 
	 * @param a
	...
	/**
	 * The <code>scale</code> method takes a <code>Vector2d</code> and a
	 * scalar float value multiplies each component of the Vector, storing the
	 * result in the third parameter.
	 * 
	 * @param a
	...
	/**
	 * The <code>normalize</code> method takes a <code>Vector2d</code> and
	 * if it is non-zero, will normalize it so that its magnitude will be 1.
	 * 
	 * @param a
	 *            the value of the vector to normalize
	...
	/**
	 * The <code>quadrant</code> method is a utility function for two
	 * dimensional vectors that takes a vector as a parameter and will return an
	 * integer describing what quadrant of the xy plane the vector lies in.
	 * 
	 * @param a
	...
	/**
	 * The <code>convert</code> methods have been overridden to allow 2d
	 * vectors to be converted to 3d vectors and vice versa.
	 * 
	 * @param src
	...
	/**
	 * The <code>distance</code> method takes two vectors and computes their
	 * (Euclidean) distance. It has been overloaded to allow the computation of
	 * the distance between a 3d vector and a 2d vector.
	 * 
	 * @param a
	 *            the value of the first vector
	 * @param b
	 ...
	 * square of their (Euclidean) distance. This is just an optimization for
	 * the <code>distance</code> method that avoids an expensive floating
	 * point square root computation. It has been overloaded to allow the
	 * computation of the distance between a 3d vector and a 2d vector.
	 * 
	 * @param a
	 *            the value of the first vector
	 * @param b
